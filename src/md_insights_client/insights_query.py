...
def c2_dns_query(api_key: str, artifacts: Union[str, List[str]]) -> dict:
    ...
    try:
        response = requests.post(f"{MD_INSIGHTS_API_HOST}/c2/dns/query", data=data)
        ...
        raise FeedAccessError(
            f"API error attempting C2 DNS query: {e}"
        )
...
def c2_ip_query(api_key: str, artifacts: Union[str, List[str]]) -> dict:
    ...
    try:
        response = requests.post(f"{MD_INSIGHTS_API_HOST}/c2/ip/query", data=data)
        response.raise_for_status()
        return response.json()
    except Exception as e:
        if hasattr(response, 'text') and response.text:
        ...
        raise FeedAccessError(
            f"API error attempting C2 IP query: {e}"
        )


def reputation_query(api_key: str, artifacts: Union[str, List[str]], 
    ...
    for chunk in _chunks(artifacts_list, max_at_a_time):
        ...
        try:
            response = requests.post(f"{MD_INSIGHTS_API_HOST}/rep/query", data=data)
            response.raise_for_status()
...
def cli():
    ...
    # C2 DNS subcommand
    c2_dns_parser = subparsers.add_parser("c2-dns", help="Query C2 DNS endpoints")
